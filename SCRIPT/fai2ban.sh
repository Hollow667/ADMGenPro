...
[[ "$apache" != "" ]] && s4="apache"
echo -e "${cor[5]} $(fun_trans "Este  es Fail2ban Protección")"
echo -e "${cor[5]} $(fun_trans "Óptimo para proteger contra ataques de DDos")"
echo -e "$barra"
if [[ ! -z "$pid_fail" ]]; then
 echo -e "${cor[2]} [1] >${cor[5]} $(fun_trans "Desinstalar Fail2ban")"
 echo -e "${cor[2]} [2] >${cor[5]} $(fun_trans "Mirar el log")"
 echo -e "${barra}"
  while [[ -z ${logxyz} || ${logxyz} != @(1|2) ]]; do
   echo -ne "\033[1;37m$(fun_trans "Selecione una Opción"): " && read logxyz
   tput cuu1 && tput dl1
  done
 case ${logxyz} in
...
echo -e "$barra"
  while [[ -z ${fail2ban} || ${fail2ban} != @(s|S|n|N|y|Y) ]]; do
   echo -ne "\033[1;37m$(fun_trans "Selecione una Opción") [S/N]: " && read fail2ban
   tput cuu1 && tput dl1
  done
